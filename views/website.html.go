...
<p><strong  class="btn btn-primary" x-show="!show" @click="show=true" x-transition>Me Right Now</strong>
<strong><img src="./assets/website/harold.jpg" alt="Me Right Now"  class="mx-auto h-full" x-show="show" x-transition></strong></p>
`)
...
<p>With HTMX the web contents are rendered on the server which means that <strong>it is possible to use any backend language</strong> as long as the HTML contents are delivered to the client through the HTTP response.</p>
<br>
...
<br>
<p>In my opinion it also requires <strong  class="bg-purple-200" >less effort to parse the server fetched data into ready-to-use HTML directly from the backend rather than exposing a json data API that need to be consumed and parsed from the frontend</strong>.</p>
...
<br>
<p><strong  class="bg-cyan-200" >These template functions and structs are compiled in the final binary</strong>: it is possible to run the final executable binary without any additional <strong><code>.html</code></strong> file dependency in the directory.</p>
<br>
