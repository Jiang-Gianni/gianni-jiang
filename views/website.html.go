...
// <style>.bg-teal-300{--bg-opacity:1;background-color:#81e6d9;background-color:rgba(129,230,217,var(--bg-opacity))}.list-disc{list-style-type:disc}.list-inside{list-style-position:inside}.bg-orange-200{--bg-opacity:1;background-color:#feebc8;background-color:rgba(254,235,200,var(--bg-opacity))}.bg-green-200{--bg-opacity:1;background-color:#c6f6d5;background-color:rgba(198,246,213,var(--bg-opacity))}.bg-blue-200{--bg-opacity:1;background-color:#bee3f8;background-color:rgba(190,227,248,var(--bg-opacity))}.mx-auto{margin-left:auto;margin-right:auto}.h-full{height:100%}.bg-yellow-200{--bg-opacity:1;background-color:#fefcbf;background-color:rgba(254,252,191,var(--bg-opacity))}.animate-pulse{animation:pulse 2s cubic-bezier(.4,0,.6,1) infinite}@keyframes pulse{50%{opacity:.5}}.bg-gray-200{--bg-opacity:1;background-color:#edf2f7;background-color:rgba(237,242,247,var(--bg-opacity))}.bg-red-200{--bg-opacity:1;background-color:#fed7d7;background-color:rgba(254,215,215,var(--bg-opacity))}.bg-purple-200{--bg-opacity:1;background-color:#e9d8fd;background-color:rgba(233,216,253,var(--bg-opacity))}.bg-indigo-200{--bg-opacity:1;background-color:#c3dafe;background-color:rgba(195,218,254,var(--bg-opacity))}.animate-bounce{animation:bounce 1s infinite}@keyframes bounce{0%,100%{transform:translateY(-25%);animation-timing-function:cubic-bezier(.8,0,1,1)}50%{transform:none;animation-timing-function:cubic-bezier(0,0,.2,1)}}.font-bold{font-weight:700}.border-8{border-width:8px}.border-teal-400{--border-opacity:1;border-color:#4fd1c5;border-color:rgba(79,209,197,var(--border-opacity))}.text-red-700{--text-opacity:1;color:#c53030;color:rgba(197,48,48,var(--text-opacity))}.bg-teal-200{--bg-opacity:1;background-color:#b2f5ea;background-color:rgba(178,245,234,var(--bg-opacity))}</style>
...
<p>You can jump directly to the section of the project you want to see by clicking the <!-- class="bg-teal-300 rounded-s" --><strong>menu on the left</strong></p>
<br>
<h2 id="chatg-stack">CHATG Stack</h2>
<p>To create this website I used the following tech stack:</p>
<ul  class="list-disc list-inside" >
<li><em  class="bg-orange-200" >CockroachDB</em></li>
<li><em  class="bg-green-200" >HTMX</em></li>
<li><em  class="bg-green-200" >AlpineJS</em></li>
<li><em  class="bg-green-200" >TailwindCSS</em></li>
<li><em  class="bg-blue-200" >Go</em></li>
</ul>
<br>
...
<br>
<p>With Cockroach DB the database cluster can be created in different regions in the world and they offer a great serverless free base plan: <strong  class="bg-green-200" >10 GB and 50M request units per month</strong>.</p>
<br>
<p>(Un)fortunately this <del>destined to fail</del> website won't get many views and I won't ever reach that cap, so I don't have to worry about it.</p>
<br>
<div x-data="{show:false}"><div>
<p><strong  class="btn btn-primary" x-show="!show" @click="show=true" >Me Right Now</strong>
...
<br>
<p>If you right click on the menu on the left and select 'Inspect' you can see that there are some attributes like <strong><code  class="bg-yellow-200" >hx-get</code></strong> and <strong><code>hx-target</code></strong>. A click on the element will trigger a GET HTTP request to the endpoint value: <strong>the corresponding HTTP response contents will then substitute the element defined by the <code>hx-target</code> attribute</strong>.</p>
<br>
<p>This means that the HTTP response body must be in form of HTML.</p>
<br>
<p><strong  class="bg-orange-200" >Any HTML element can trigger an HTTP request and all the HTTP verbs can be used (POST, GET, PUT, PATCH, and DELETE)</strong> in contrast to plain HTML where only POST (with 'form' tags) and GET (with 'a' tags) are possible.</p>
<br>
<h2 id="javascript-size">JavaScript size</h2>
<p>There is no need on the client side to manipulate the response, <strong  class="bg-cyan-200" >no json to be parsed</strong>: this means that the page will load faster since <strong>there is no javascript (other than htmx) to be executed</strong>.</p>
...
<br>
<p>Now try to do the same on <strong><a href="https://create-react-template.vercel.app"  target="_blank" class="animate-pulse bg-gray-200" >https://create-react-template.vercel.app</a></strong></p>
<br>
...
<p>No need for any <strong><code>npm install ...</code></strong> which can easily cause the local <strong><code>node_modules</code></strong> folder to exceed 100 MB of size when using frameworks like React.</p>
<p><strong><img src="./assets/website/node-modules.png" alt="Node Modules"  class="mx-auto h-full" ></strong></p>
<br>
<h2 id="server-side-rendering">Server Side Rendering</h2>
<p>With HTMX the web contents are rendered on the server which means that <strong>it is possible to use any backend language</strong>: all the HTML contents are delivered to the client through the HTTP response.</p>
<br>
...
<br>
<p>In my opinion it also requires <strong  class="bg-purple-200" >less effort to parse the server fetched data into ready-to-use HTML directly from the backend rather than exposing a json data API that needs to be consumed and parsed from the frontend</strong>.</p>
<p>Same goes for managing <strong  class="bg-indigo-200" >the state of the application</strong>, most of the times.</p>
<br>
`)
//line views/website.html:93
}
...
<br>
<p>Combining Alpine JS, HTMX and Tailwind CSS (next section) makes it possible to generate the following component <strong  class="bg-indigo-200" >just by adding attributes to HTML elements</strong>.</p>
`)
//line views/website.html:107
	StreamAlpineCountToggle(qw422016)
//line views/website.html:107
	qw422016.N().S(`
`)
//line views/website.html:108
}
...
<br>
<p>It lets you add some classes to an HTML element rather than to write the element selector (tag / id / class) and its styling in a css file or inside a style tag: the HTML element gets more congested (especially with HTMX and Alpine) but I prefer writing 3-5 lines of attributes and values rather than to write 10-20 lines (or more) of CSS or JS (I am not fond of either).</p>
...
<br>
<p>What I really find cool is that <strong  class="bg-green-200" >it also auto generates custom Go structs in case the query consists of extracting columns from multiple different tables to match the corresponding fields and types</strong>: you end up having the structs and the query functions with type safety without writing any Go boilerplate code.</p>
<br>
...
<p>Go already has a template package in the standard library to generate text but I don't like that some of the expressions aren't Go-like: for example the 'if' equivalent in the template is <strong><code>{{if condition}} Stuff {{end}}</code></strong> and if you want to use a custom written Go function that returns a boolean as the <code>condition</code> then you need to put it in a map from Go, pass the map to the Go template variable and call the map key in the if condition inside the curly braces.</p>
<br>
<p><strong><a href="https://github.com/valyala/quicktemplate"  target="_blank" >https://github.com/valyala/quicktemplate</a></strong></p>
<br>
<p><strong  class="bg-orange-200" >quicktemplate addresses this by letting you use the Go language inside the curly braces in the template files.</strong></p>
<br>
<p>In the template files you can define functions, structs and insert some raw Go code.</p>
<p><strong  class="bg-yellow-200" >The tool (executed from the command line) then generates .go files that contains all the templates definitions (functions, structs etc.)</strong> and can be exported and used inside the project.</p>
<br>
<p><strong  class="bg-cyan-200" >These template functions and structs are compiled in the final binary</strong>: it is possible to run the final executable binary and start the server without being dependent on any <strong><code>.html</code></strong> file in the directory that needs to be read and parsed (with all the performance implications).</p>
<br>
...
<br>
<h2 id="chi">chi</h2>
<p>chi is a lightweight, idiomatic and composable router for building Go HTTP services.</p>
<p><strong><a href="https://go-chi.io/#/README"  target="_blank" >https://go-chi.io/#/README</a></strong></p>
<br>
<p>It makes it easy to group subroutes and to apply some common middlewares.</p>
<br>
<p>What I also like about chi is the <a href="https://pkg.go.dev/github.com/go-chi/chi#Walk"><strong  target="_blank" class="animate-pulse" >Walk</strong></a> function: combining this with the standard library <a href="https://pkg.go.dev/reflect"><strong  target="_blank" class="animate-pulse" >reflect</strong></a> and <a href="https://pkg.go.dev/runtime"><strong  target="_blank" class="animate-pulse" >runtime</strong></a> packages makes it easy to <strong  class="bg-teal-200" >automatically generate the documentation of the application's APIs with all the exact files and lines references in the codebase</strong>.</p>
<br>
<p>Example for this application: <strong><a href="https://github.com/Jiang-Gianni/gianni-jiang/blob/main/docRoutes.md"  target="_blank" >https://github.com/Jiang-Gianni/gianni-jiang/blob/main/docRoutes.md</a></strong></p>
<br>
`)
...
<br>
<p>Click the <strong  class="btn btn-accent" >Create New Todo</strong> button to generate a new Todo or click one of the Todos in the table if you want to <strong  class="btn btn-primary" >update</strong> or to <strong  class="btn btn-error" >delete</strong> it.</p>
<br>
`)
//line views/website.html:172
	StreamTodoDemo(qw422016)
//line views/website.html:172
	qw422016.N().S(`
`)
//line views/website.html:173
}
...
//line views/website.html:175
func StreamWebsiteDemoNumber(qw422016 *qt422016.Writer) {
//line views/website.html:175
	qw422016.N().S(`
<p>Simple HTTP call that retrieves some trivia from <a href="http://numbersapi.com"><strong  class="bg-blue-200 animate-pulse" >numbersapi</strong></a> of a random number.</p>
<br>
`)
//line views/website.html:178
	StreamNumbersApi(qw422016)
//line views/website.html:178
	qw422016.N().S(`
<br>
`)
//line views/website.html:180
}
...
<p><strong  class="bg-red-200" >The downside is that Serverless Function have a maximum execution duration, so it is not possible to maintain a WebSocket connection.</strong></p>
<br>
<p><strong><a href="https://vercel.com/docs/concepts/functions/serverless-functions/runtimes/go"  target="_blank" class="animate-pulse bg-gray-200" >https://vercel.com/docs/concepts/functions/serverless-functions/runtimes/go</a></strong></p>
<br>
...
<p><strong  class="bg-red-200" >The downside is that if the server doesn't receive any request for a period of time then the execution stops and the cold start may take time (even more than 10 seconds).</strong></p>
<br>
<p><strong><a href="https://render.com/docs"  target="_blank" class="animate-pulse bg-gray-200" >https://render.com/docs</a></strong></p>
<br>
`)
//line views/website.html:200
}
